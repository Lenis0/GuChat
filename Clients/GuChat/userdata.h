...
public:
    AddFriendApply(int from_uid, QString name, QString desc,
                   QString icon, QString nickname, int sex);
    int _from_uid;
    QString _name;
    QString _desc;
    QString _icon;
...
struct ApplyInfo {
    ApplyInfo(int uid, QString name, QString desc,
        QString icon, QString nickname, int sex, int status)
        :_uid(uid),_name(name),_desc(desc),
        _icon(icon),_nickname(nickname),_sex(sex),_status(status){}
    ...
    ApplyInfo(std::shared_ptr<AddFriendApply> addinfo)
        :_uid(addinfo->_from_uid),_name(addinfo->_name),
          ...
          _nickname(addinfo->_nickname),_sex(addinfo->_sex),
          _status(0)
    {}
...
struct AuthInfo {
    AuthInfo(int uid, QString name,
             QString nickname, QString icon, int sex):
        _uid(uid), _name(name), _nickname(nickname), _icon(icon),
        _sex(sex){}
    int _uid;
    QString _name;
    QString _nickname;
...
struct AuthRsp {
    AuthRsp(int peer_uid, QString peer_name,
            QString peer_nickname, QString peer_icon, int peer_sex)
        :_uid(peer_uid),_name(peer_name),_nickname(peer_nickname),
...
struct TextChatData;
struct FriendInfo {
    FriendInfo(int uid, QString name, QString nickname, QString icon,
        int sex, QString desc, QString back, QString last_msg=""):_uid(uid),
        _name(name),_nickname(nickname),_icon(icon),_sex(sex),_desc(desc),
        _back(back),_last_msg(last_msg){}

    FriendInfo(std::shared_ptr<AuthInfo> auth_info):_uid(auth_info->_uid),
    _nickname(auth_info->_nickname),_icon(auth_info->_icon),_name(auth_info->_name),
      _sex(auth_info->_sex){}

    FriendInfo(std::shared_ptr<AuthRsp> auth_rsp):_uid(auth_rsp->_uid),
    _nickname(auth_rsp->_nickname),_icon(auth_rsp->_icon),_name(auth_rsp->_name),
      _sex(auth_rsp->_sex){}
...
struct UserInfo {
    ...
    UserInfo(std::shared_ptr<AuthInfo> auth):
        _uid(auth->_uid),_name(auth->_name),_nickname(auth->_nickname),
        _icon(auth->_icon),_sex(auth->_sex),_last_msg(""){}

    UserInfo(int uid, QString name, QString icon):
    _uid(uid), _name(name), _icon(icon),_nickname(_name),
    _sex(0),_last_msg(""){
