...
bool ContactUserList::eventFilter(QObject* watched, QEvent* event) {
    ...
    if (watched == this->viewport() && event->type() == QEvent::Wheel) {
        ...
        if (maxScrollValue - currentValue <= 0) {
            // auto b_loaded = UserMgr::GetInstance()->IsLoadChatFin(); // 测试 待优化
            bool b_loaded = false;
            if (b_loaded) {
                return true;
            }
...
void ContactUserList::addContactUserList() {
    ...
    this->setItemWidget(add_item, _add_friend_item); // item加入list 用替换的方式
    //默认设置新的朋友申请条目被选中
...
// void ContactUserList::slot_add_auth_firend(std::shared_ptr<AuthInfo>) {
