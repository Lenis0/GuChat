...
SearchInfo::SearchInfo(int uid, QString name,
                       QString nickname, QString desc, int sex, QString icon):_uid(uid)
    ,_name(name), _nickname(nickname),_desc(desc),_sex(sex),_icon(icon){
}
...
AddFriendApply::AddFriendApply(int from_uid, QString name, QString desc,
    ...
    _desc(desc),_icon(icon),_nickname(nickname),_sex(sex)
{
