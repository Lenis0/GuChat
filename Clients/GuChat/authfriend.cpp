...
AuthFriend::AuthFriend(QWidget* parent):
    QDialog(parent), ui(new Ui::AuthFriend), _label_point(2, 6) {
    ui->setupUi(this);
    // 隐藏对话框标题栏
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &CustomEdit::returnPressed, this, &AuthFriend::SlotLabelEnter);
    connect(ui->lb_ed, &CustomEdit::textChanged, this, &AuthFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &CustomEdit::editingFinished, this, &AuthFriend::SlotLabelEditFinished);
...
void AuthFriend::InitTipLbs() {
    ...
    for (size_t i = 0; i < _tip_data.size(); i++) {
        ...
        connect(lb, &ClickableLabel::sig_clicked, this, &AuthFriend::SlotChangeFriendLabelByTip);
...
//点击标已有签添加或删除新联系人的标签
void AuthFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickLabelState state) {
