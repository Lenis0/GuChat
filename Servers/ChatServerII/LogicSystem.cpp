...
void LogicSystem::PostMsgToQue(std::shared_ptr<LogicNode> msg) {
	...
	_msg_que.push(msg);
	// 由0变为1则发送通知信号
	// 优化空间 ①如果处理消息队列速度太慢 队列已经达到了最大值 可以直接返回
	// ②可以有多个线程 多个队列 投入到不同的队列中
	if (_msg_que.size() == 1) {
		unique_lk.unlock();
		_consume.notify_one();
	}
...
void LogicSystem::LoginHandler(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data) {
	...
	rtvalue["nickname"] = user_info->nickname;
	...
	for (auto& friend_ele : friend_list) {
		...
		obj["nickname"] = friend_ele->nickname;
...
void LogicSystem::SearchInfo(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data) {
	...
	Json::Value root;
	reader.parse(msg_data, root);
	auto search_str = root["search"].asString();
	std::cout << "user SearchInfo uid is  " << search_str << std::endl;
	...
	bool b_digit = isPureDigit(search_str);
	if (b_digit) {
		GetUserByUid(search_str, rtvalue);
	} else {
		GetUserByName(search_str, rtvalue);
	}
	return;
}
...
void LogicSystem::AddFriendApply(std::shared_ptr<CSession> session, const short& msg_id, const std::string& msg_data) {
	...
	reader.parse(msg_data, root);
	auto uid = root["uid"].asInt();
	auto applydesc = root["applydesc"].asString();
	auto backname = root["backname"].asString();
	auto touid = root["touid"].asInt();
	std::cout << "user login uid is  " << uid << " applydesc  is "
		<< applydesc << " backname is " << backname << " touid is " << touid << std::endl;
	...
	if (to_ip_value == self_name) {
		...
		if (session) {
			...
			notify["applyuid"] = uid;
			notify["applydesc"] = applydesc; // 申请信息 （我是xxx）
			if (b_info) {
				notify["name"] = apply_info->name;
				notify["icon"] = apply_info->icon;
				notify["sex"] = apply_info->sex;
				notify["nickname"] = apply_info->nickname;
			}
	...
	add_req.set_desc(applydesc);
	if (b_info) {
		add_req.set_name(apply_info->name);
		add_req.set_icon(apply_info->icon);
		add_req.set_sex(apply_info->sex);
		add_req.set_nickname(apply_info->nickname);
	}
	...
	if (to_ip_value == self_name) {
		...
		if (session) {
			...
			if (b_info) {
				...
				notify["nickname"] = user_info->nickname;
...
void LogicSystem::GetUserByUid(std::string uid_str, Json::Value& rtvalue) {
	...
	if (b_base) {
		...
		auto nickname = root["nickname"].asString();
	...
	redis_root["nickname"] = user_info->nickname;
